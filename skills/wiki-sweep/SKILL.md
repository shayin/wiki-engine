...
精析时按类型映射到对应扫描维度：OVERDUE_DECISION → C，MISSING_FOLLOWUP → A，STALE_FOLLOWUP → A，STALE_TOPIC → E，ORPHAN_SOURCE → G，BROKEN_LINK → H，TAG_STATS → F，TODO_MISPLACED → I，STALE_DATA → J。
...
- [ ] E. [拼多多] 3 篇新文章未关联到 topic `type: 维护`
- [ ] F. "PDD"和"拼多多"标签碎片 `type: 维护`
```
