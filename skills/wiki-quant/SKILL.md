...
## 🔴 强制规则（2026-08-06 新增）—— wiki-research 调用最小必跑集

wiki-research 决策型个股研究调用本 skill 时，**必须跑以下模式**，不能只跑 indicators 就算技术面完成：
- **模式1（detail 快速扫描）**：形态信号（trend_template/VCP/杯柄等）——揭示趋势阶段，**禁止只讲"突破均线"不讲 Stage 阶段**
- **模式4（factors eval）+ 4b（matrix）**：Alpha IC/IR/多周期 + `ic_to_factor_weight` 拿因子权重
- **模式5（出场策略 ExitStrategySignal）**：已建仓标的必填——止损止盈价位来自工具，不能凭主观
- **模式6（仓位 position_size + portfolio_risk_check）**：建议买入/已建仓必填——Murphy 2% + 组合风控
- **模式8（regime-eval）**：因子 regime 自适应（港股可能 N/A，标局限不跳过）

**禁止**：只跑 `indicators`（22 指标）+ `factors eval` 就算技术面完成——会漏掉形态（趋势阶段判断）、出场（止损止盈）、仓位（风控）、regime（因子有效性），导致模板填空式敷衍。

**变量分层对齐走势**：报告「主导变量」必须区分「当前走势驱动」(短期) vs「长期结构」，主导变量和股价走势方向一致（走势反弹时主导不能是"失血"）。详见 CLAUDE.md「个股深度报告技术面全套 + 变量分层对齐走势」强制规则。

**历史教训（2026-08-06 美团）**：用户批评"技术面才几个指标、没形态分析、变量与走势脱节、敷衍"。根因：只 indicators+factors，跳 detail(形态)/模式5(出场)/模式6(仓位)/模式8(regime)；变量说"结构性失血主导"但走势+36%反弹矛盾。修正：补 trend_template 0.50（底部反弹非趋势确认）/出场 WATCH/中概集中度59%/变量分层对齐走势。
...
**背离 = 价格走一个方向，多空力量/指标走另一个方向 = 趋势转折预警**。决策型个股研究必检测。

**两类**：
- **底背离**：价格创新低，但多空综合分/RSI/MACD 不创新低（抬高）→ 见底信号
- **顶背离**：价格创新高，但指标不创新高（降低）→ 见顶信号

**检测方法**（配合模式15 趋势图，上下图对照）：
1. 找价格近期低点/高点（K线图，上图）
2. 对照同时点的多空综合分面积（下图）/ RSI / MACD
3. 价格新低 + 多空分/RSI 抬高 = 底背离；价格新高 + 指标降低 = 顶背离

**全景趋势图（模式15 升级）**：从纯趋势图升级为**全景版**，一个 HTML 三部分：
1. **当前多空快照**（8 指标表 + 多空比 bar + "谁占上风"结论）
2. **K线 + 多空分趋势**（两 grid 上下对照：上图 K线+均线，下图 多空综合分面积+DI，axisPointer 联动看背离）
3. **图例**（每条线/区域什么意思）

**强制规则**：
- 决策型个股研究**必须检测背离**（价格 vs 多空综合分 + RSI + MACD）
- 发现背离 → 报告技术面节显式标注（底/顶背离 + 位置 + 含义）+ 全景趋势图标注背离点
- 背离是转折预警，不是确认信号 → 配合突破/量验证

**历史（2026-08-06 美团）**：6 月底 K 线创新低(64-68)，但多空综合分面积红区已收窄（空头撤退）= **底背离** → 随后反弹 +36%。当前价格卡 MA250 但多空分强多头 = 力量领先价格（待突破确认）。
