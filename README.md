...
| `wiki-research` | 说"研究"/"研"/"调研"/"了解"/"刷新"/"重新研究" | **两种模式**：新课题（开题→搜集→分析→报告→入库→跟进）或 已有课题刷新（读基线→搜集→深度分析[挖盲区+反面论据+透镜+二级效应]→delta report→tracker 更新）。深度等同，自动识别。 |
| `wiki-todo` | 说"加个待办"/"完成了"/"看看待办" | 添加、完成、删除、编辑、归档待办 |
| `wiki-review` | 说"复盘"/"回顾"，或 cron | 扫描到期决策，逐条复盘，记录教训 |
| `wiki-sweep` | 说"扫一下"/"检查遗漏"/"整理知识库"，或 cron，或 `/wiki-sweep 拼多多`（单标的） | 两档全自动维护（A 机械修复 + C 判断更新）+ 变量状态查证 + 微信推送 |
| `wiki-learn` | 说"学习XX"/"过一遍XX"/"看看XX相关内容"/"继续学习"/"看到哪了" | 把知识库内容组织成有序阅读路径，追踪进度 |
| `wiki-push` | 说"推送到微信"/"发微信"/"微信通知" | 推送消息到微信，支持空内容追问和错误分类 |

**补充说明**：
- `wiki-mine` 是**内部 skill**，不对用户暴露。被 wiki-sweep（维度 M）和 wiki-research（步骤 3）自动调用。用户想挖盲区请用 `/wiki-sweep {标的}`，sweep 内部会自动调 mine。
- `wiki-lens` 和 `wiki-write` 用户可直接调用，也会被其他 skill 自动集成。
